...
function countAllPeople() {
  let count = got.houses.reduce((acc, x) => acc + x.people.length, 0);
  return count;
}

function peopleByHouses() {
  return got.houses.reduce((acc, x) => {
    acc[x.name] = x.people.length;
    return acc;
  }, {});
}
...
function everyone() {
  return got.houses.reduce((acc, x) => {
    x.people.forEach((y) => acc.push(y.name));
    return acc;
  }, []);
}
